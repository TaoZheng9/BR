...
class AudioVisualModel(torch.nn.Module):
    ...
    def forward(self, input, volatile=False):
        ...
        ratio_predictionAbs=Variable((ratio_predictionVl[:,0,]**2+ratio_predictionVl[:,1,]**2).unsqueeze(1)**0.5,requires_grad=False)
        ratio_prediction=ratio_predictionVl*ratioMag_pred/(ratio_predictionAbs+eps)
